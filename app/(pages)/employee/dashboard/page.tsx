        ...
        <div className="bg-gradient-to-br from-red-600 via-red-600 to-red-700 px-8 py-6">
          <DialogHeader>
            ...
            <p className="text-red-100 mt-2 text-sm">
              ...
              <span className="font-medium">"{fileName}"</span>? This action
              cannot be undone.
            </p>
          </DialogHeader>
        </div>
        ...
        <DialogFooter className="px-8 py-6 bg-slate-50 border-t border-slate-200">
          <div className="flex justify-end space-x-3 w-full">
            <Button
              variant="outline"
              onClick={onCancel}
              className="h-11 cursor-pointer px-6 rounded-xl border-2 border-slate-300 hover:border-slate-400 font-medium transition-all duration-200"
              disabled={saving}
            >
              Cancel
            </Button>
...
const EmployeeDetailPage: React.FC = () => {
  ...
  const fetchEmployeeDetails = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/emploee/current`,
        {
          method: "GET",
          headers: {
  ...
  const handleUploadFile = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/emploee/upload`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <ConfirmDeleteModal
        fileId={deleteFileInfo?.fileId || ""}
        fileName={deleteFileInfo?.fileName || ""}
